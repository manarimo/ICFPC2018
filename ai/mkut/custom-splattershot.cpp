...
struct Reservation {
    ...
    bool isReservedFuture(const Point& pos, int time) {
        for (int i = 0; i < eternal.size(); i++) {
            if (eternal[i].first == pos) return true;
        }
        for (int i = time; i < reserved.size(); i++) {
            if (reserved[i].count(pos)) return true;
        }
        return false;
    }
...
vector<string> find_path(int W, vector<vector<vector<char> > >& currentModel, Point& s, set<Point> t, Reservation& reservation) {
    ...
    while (q.size()) {
        ...
        if (p.t > 10) break;
        if (t.count(p.p) && !reservation.isReservedFuture(p.p, p.t)) {
        ...
        for (int i = 0; i < NUM_ADJ; i++) {
            for (int j = 1; j <= 15; j++) {
                ...
                if (!prev.count(ntp)) {
                    ...
                    q.push(ntp);
                    if (t.count(np) && !reservation.isReservedFuture(np, p.t + 1)) {
                        found = true;
                    }
                }
            }
                ...
                for (int i2 = 0; i2 < 6; i2++) {
                    ...
                    for (int k2 = 1; k2 <= 5; k2++) {
                        ...
                        if (!prev.count(ntp)) {
                            ...
                            q.push(ntp);
                            if (t.count(np2) && !reservation.isReservedFuture(np2, p.t + 1)) {
                                found = true;
                            }
                        }
                    }
...
vector<vector<vector<string> > > computePath(const int W, vector<vector<vector<char> > >& model, vector<Point>& tasks, vector<vector<Point> >& newTasks) {
    ...
    vector<Point> pos(2);
    for (int i = 0; i < 2; i++) pos[i] = bots_arrange[i] * Point(W - 1, R - 1, R - 1);
    ...
    vector<vector<string> > final(2);
    Reservation reservation(pos);
    int max_len = 0;
    for (int i = 0; i < 2; i++) {
        set<Point> goals; goals.insert((bots_arrange[i] + Point(0, 1, 0)) * Point(W - 1, R - 1, R - 1));
...
int main() {
    ...
    // Robot ID 計算
    vector<pair<int, int> > team_and_id(N * 2);
    for (int i = 0; i < N; i++) {
        team_and_id[i] = make_pair(i, 0);
    }
    for (int i = 0; i < N; i++) {
        team_and_id[N + i] = make_pair(N - 1 - i, 1);
    }

    // 準備
    for (int i = 0; i < N - 1; ) {
        for (int k = 0; k < i; k++) cout << "wait" << endl;
        cout << "fission 1 0 0 " << (N - i - 1) * 2 - 1 << endl;
        i++;
    ...
    // 散開
    {
        vector<vector<string> > sankai_command(N * 2);
        for (int i = 0; i < N * 2; i++) {
        ...
        int maxSankaiSize = 0;
        for (int i = 0; i < N * 2; i++) maxSankaiSize = max(maxSankaiSize, (int) sankai_command[i].size());
        for (int i = 0; i < N * 2; i++) while (sankai_command[i].size() < maxSankaiSize) sankai_command[i].push_back("wait");
